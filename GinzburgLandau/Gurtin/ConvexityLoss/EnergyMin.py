...
if read_in == 0: # We want to use the standard values.
 ...
 A1 = interpolate( Expression("sin(x[0])", degree=1), V)
 A2 = interpolate( Expression("sin(x[1])", degree=1), V)
...
for tt in range(NN):
 ...
 tol_test = np.linalg.norm( np.asarray(Fa1_vec.get_local()) + np.asarray(Fa2_vec.get_local()) + np.asarray(Fu_vec.get_local()) )
...
print("Please input K -ve definite, K=-I would be a good choice.")
print("================ Note ========================")
