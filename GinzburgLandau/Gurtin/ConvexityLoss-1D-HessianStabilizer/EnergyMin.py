...
# 
#2. Pi = ( (1-u**2)**2/2 + K1*inner(grad(u), grad(u)) + K2*(u.dx(0).dx(0))**2 + 2*a*u*T1*u.dx(0) + (a**2)*u**2 + inner( alpha*grad(a), alpha*grad(a) ) )*dx
#   K1=1, K2=5, T1=1.6, alpha=1.0
#======================================================================================================
##Things to do:-
#1.
#======================================================================================================
...
#Choice 2
Pi = ( (1-u**2)**2/2 + K2*(u.dx(0).dx(0))**2 + K1*inner(grad(u), grad(u))**2 + 2*a*u*T1*u.dx(0) \
        + (a**2)*u**2 + inner( alpha*grad(a), alpha*grad(a) ) )*dx
...
if read_in == 0: # We want to use the standard values.
 ...
 A = interpolate( Expression("sin(x[0])", degree=2), V)
 U = interpolate( Expression("1+0.1*sin(x[0])", degree=2), V)
...
###---------------------------------------------------------------------------------------------------------------
 ...
 A = Function(V)
 U = Function(V)
 a_in = XDMFFile("./Gurtin-AFM1D-0.xdmf")
 a_in.read_checkpoint(A,"a",0)
 u_in = XDMFFile("./Gurtin-AFM1D-1.xdmf")
...
for tt in range(NN):
 ...
 #       + (a**2)*u**2 + inner( alpha*grad(a), alpha*grad(a) ) )*dx )
 #Choice 2
 pie1 = assemble( (1/lx)*( (1-u**2)**2/2 + K2*(u.dx(0).dx(0))**2 + K1*inner(grad(u), grad(u))**2 + 2*a*u*T1*u.dx(0) \
        + (a**2)*u**2 + inner( alpha*grad(a), alpha*grad(a) ) )*dx )
...
#        + (a**2)*u**2 + inner(alpha*grad(a), alpha*grad(a) ) )*dx )
#Choice 2
pie = assemble( (1/lx)*( (1-u**2)**2/2 + K2*(u.dx(0).dx(0))**2 + K1*inner(grad(u), grad(u))**2 + 2*a*u*T1*u.dx(0) \
        + (a**2)*u**2 + inner( alpha*grad(a), alpha*grad(a) ) )*dx )
...
##Testing Hessian method
#This was used to test if the method of computing the 2n  derivative was accurate or not.
##v = interpolate( Expression("sin(3*x[0])", degree=2), V)
#v = interpolate( Expression("x[0]*x[0]*3", degree=2), V)
#
#def grad(v):
#    return v.dx(0)
#def Hessian1(v):
...
#
#Hess1 = Function(V)
#Hess1 = project(Hessian1(v))
#Hess2 = Function(V)
#Hess2 = project(Hessian2(v))
#Hess3 = Function(V)
#Hess3 = project(v.dx(0).dx(0))
#grad1 = Function(V)
#grad1 = project(grad(v))
...
plt.show()
#c = plot(Hess1)
#plt.title(r"$Hessian(x)$",fontsize=26)
#plt.show()
#c = plot(Hess2)
#plt.title(r"$Hessian(x)$",fontsize=26)
#plt.show()
...
#plt.show()
#c = plot(grad1)
#plt.title(r"$Grad(x)$",fontsize=26)
#plt.show()
