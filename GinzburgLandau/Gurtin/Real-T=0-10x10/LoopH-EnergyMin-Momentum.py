#The job of this code is to set $\theta$ to 0. and we want to solve for the domain walls
#WE have explicitly set T_m=0, in accordance with the special case in the PRL paper with pradeep.
#======================================================================================================
...
for ih in range(6,10):
 ...
 ###---------------------------------------------------------------------------------------------------------------
  ...
  A2 = Function(V)
  U = Function(V)
  a1_in = XDMFFile("H%s/Gurtin-SCIsland-0.xdmf"%int(ih))
  a1_in.read_checkpoint(A1,"a1",0)
  a2_in = XDMFFile("H%s/Gurtin-SCIsland-1.xdmf"%int(ih))
  a2_in.read_checkpoint(A2,"a2",0)
  u_in = XDMFFile("H%s/Gurtin-SCIsland-2.xdmf"%int(ih))
 ...
 a1a2tu_out = XDMFFile('H%s/Gurtin-SCIsland-0.xdmf'%int(ih))
 a1a2tu_out.write_checkpoint(a1, "a1", 0, XDMFFile.Encoding.HDF5, False) #false means not appending to file
 pvd_file = File("H%s/Gurtin-SCIsland-0.pvd"%int(ih)) # for paraview. 
 pvd_file << a1
 a1a2tu_out.close()
 a1a2tu_out = XDMFFile('H%s/Gurtin-SCIsland-1.xdmf'%int(ih))
 a1a2tu_out.write_checkpoint(a2, "a2", 0, XDMFFile.Encoding.HDF5, False) #false means not appending to file
 pvd_file = File("H%s/Gurtin-SCIsland-1.pvd"%int(ih)) # for paraview. 
 pvd_file << a2
 a1a2tu_out.close()
 a1a2tu_out = XDMFFile('H%s/Gurtin-SCIsland-2.xdmf'%int(ih))
 a1a2tu_out.write_checkpoint(u, "u", 0, XDMFFile.Encoding.HDF5, False) #false means not appending to file
 pvd_file = File("H%s/Gurtin-SCIsland-2.pvd"%int(ih)) # for paraview.
 pvd_file << u
 a1a2tu_out.close()
 a1a2tu_out = XDMFFile('H%s/Gurtin-SCIsland-h.xdmf'%int(ih))
 a1a2tu_out.write_checkpoint(h, "h", 0, XDMFFile.Encoding.HDF5, False) #false means not appending to file
 pvd_file = File("H%s/Gurtin-SCIsland-h.pvd"%int(ih)) # for paraview.
 pvd_file << h
 a1a2tu_out.close()
 ...
 cb = plt.colorbar(c)
 plt.savefig('H%s/u(x)-H%s.png'%(int(ih),int(ih)))
 plt.clf()
 c = plot(h)
 plt.title(r"$h(x)$",fontsize=26)
 cb = plt.colorbar(c)
 plt.savefig('H%s/h(x)-H%s.png'%(int(ih),int(ih)))
 plt.clf()
