...
#WE modified the previous code (which had theta=0) to make the problem 2D.
...
ly = 10
kappa = Constant(2.0)
N = int(np.ceil(lx*20/kappa))
mesh = RectangleMesh(Point(0.0, 0.0), Point(lx, ly), N, N)
...
H3 = float(np.sqrt(2)) #Constant(input("H3? -->"));
tau = Constant(0.1) #Constant(input("tau? -->"));
lmbda = Constant(1.0) #Constant(input("Lame parameter lambda? -->"));
mu = Constant(2.0) #Constant(input("Lame parameter mu? -->"));
Gamma1 = Constant(1.0) # Constant(input("gamma1? -->")); #Gamma is \gamma in the notes. 2 distinguish from learning rate.
Gamma2 = Constant(1.0) #Constant(input("gamma2? -->"));
s11 = Constant(input("s11? -->")); #Constant(1.0) #s is the externally applied stress.
s12 = Constant(input("s12? -->")); #Constant(1.0) 
s22 = Constant(input("s22? -->")); #Constant(1.0) 
tol = float(input("absolute tolerance? --> "))
read_in = int(input("Read from file? 1 for Yes, 0 for No --> "))
ell = float(10) #Augmented lagrangian to ensure \int 2u-1 = 0.
...
#Defining the energy
Pi = ( (1-u**2)**2/2 + inner(grad(u),grad(u)) + 2*tau*(u.dx(0)*a1*u + u.dx(1)*a2*u ) \
      + ( a1**2 + a2**2 )*u*u  + 0.5*lmbda*( eps11(v1) + eps22(v2) )**2 \
      + mu*( eps11(v1)**2 + eps22(v2)**2 + 2*eps12(v1,v2)**2 ) - ( s11*eps11(v1) + s22*eps22(v2) + 2*s12*eps12(v1,v2) ) \
      + Gamma1*( eps11(v1)*a1*u + eps12(v1,v2)*a2*u ) + Gamma2*( eps12(v1,v2)*a1*u + eps22(v2)*a2*u ) \
      + (a2.dx(0)-a1.dx(1) - H3)**2 )*dx #+ ell*(2*u-1)**2  
...
###---------------------------------------------------------------------------------------------------------------
 ...
 V2 = Function(V)
 U = Function(V)
 a1_in =  XDMFFile("Gr-iso-strain-theta0-2D-0.xdmf")
 a1_in.read_checkpoint(A1,"a1",0)
 a2_in =  XDMFFile("Gr-iso-strain-theta0-2D-1.xdmf")
 a2_in.read_checkpoint(A2,"a2",0)
 v1_in =  XDMFFile("Gr-iso-strain-theta0-2D-2.xdmf")
 v1_in.read_checkpoint(V1,"v1",0)
 v2_in =  XDMFFile("Gr-iso-strain-theta0-2D-3.xdmf")
 v2_in.read_checkpoint(V2,"v2",0)
 u_in =   XDMFFile("Gr-iso-strain-theta0-2D-4.xdmf")
...
for tt in range(NN):
 ...
 tol_test = np.linalg.norm(np.asarray(Fa1_vec.get_local()) + np.asarray(Fa2_vec.get_local()) + np.asarray(Fv1_vec.get_local()) + np.asarray(Fv2_vec.get_local())\
          + np.asarray(Fu_vec.get_local()))
 #print(tol_test)
...
avf_out = XDMFFile('Gr-iso-strain-theta0-2D-0.xdmf')
avf_out.write_checkpoint(a1, "a1", 0, XDMFFile.Encoding.HDF5, False) #false means not appending to file
pvd_file = File("Gr-iso-strain-theta0-2D-0.pvd") # for paraview. 
pvd_file << a1
avf_out.close()

avf_out = XDMFFile('Gr-iso-strain-theta0-2D-1.xdmf')
avf_out.write_checkpoint(a2, "a2", 0, XDMFFile.Encoding.HDF5, False) #false means not appending to file
pvd_file = File("Gr-iso-strain-theta0-2D-1.pvd") # for paraview. 
pvd_file << a2
avf_out.close()

avf_out = XDMFFile('Gr-iso-strain-theta0-2D-2.xdmf')
avf_out.write_checkpoint(v1, "v1", 0, XDMFFile.Encoding.HDF5, False) #false means not appending to file
pvd_file = File("Gr-iso-strain-theta0-2D-2.pvd") # for paraview. 
pvd_file << v1
avf_out.close()

avf_out = XDMFFile('Gr-iso-strain-theta0-2D-3.xdmf')
avf_out.write_checkpoint(v2, "v2", 0, XDMFFile.Encoding.HDF5, False) #false means not appending to file
pvd_file = File("Gr-iso-strain-theta0-2D-3.pvd") # for paraview. 
pvd_file << v2
avf_out.close()

avf_out = XDMFFile('Gr-iso-strain-theta0-2D-4.xdmf')
avf_out.write_checkpoint(u, "u", 0, XDMFFile.Encoding.HDF5, False) #false means not appending to file
pvd_file = File("Gr-iso-strain-theta0-2D-4.pvd") 
pvd_file << u
avf_out.close()


pie = assemble((1/(lx*ly))*( (1-u**2)**2/2 + inner(grad(u),grad(u)) + 2*tau*(u.dx(0)*a1*u + u.dx(1)*a2*u ) \
      + ( a1**2 + a2**2 )*u*u  + 0.5*lmbda*( eps11(v1) + eps22(v2) )**2 \
      + mu*( eps11(v1)**2 + eps22(v2)**2 + 2*eps12(v1,v2)**2 ) \
      - ( s11*eps11(v1) + s22*eps22(v2) + 2*s12*eps12(v1,v2) ) \
      + Gamma1*( eps11(v1)*a1*u + eps12(v1,v2)*a2*u ) \
      + Gamma2*( eps12(v1,v2)*a1*u + eps22(v2)*a2*u ) \
      + (a2.dx(0)-a1.dx(1) - H3)**2)*dx)
...
plt.show()
c = plot(v1)
plt.title(r"$v_1(x)$",fontsize=26)
plt.colorbar(c)
plt.show()
c = plot(v2)
plt.title(r"$v_2(x)$",fontsize=26)
plt.colorbar(c)
plt.show()
