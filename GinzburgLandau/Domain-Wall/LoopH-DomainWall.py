#This code is the looping version of DomainWall.py. 
#Here we loop over H from 0 to 1 in 0.1 increments,
#and plot the surface energy for the domain wall versus direction for each.
#We save them in the corresponding folder.
...
for Hl in np.arange(0, 1.0, 0.1):
   ...
   for mu in mu_values:
       ...
       if mu == 0: # We want to use the standard values.
        ...
        Aur = interpolate( Expression(('0', '1', '11'), ul=ul, ur=ur, Al=Al, Ar=Ar, Lx=Lx, degree=2), V)
       ###---------------------------------------------------------------------------------------------------------------
        ...
        u = Function(Vcoord)
        r = Function(RFnSp)
        data = np.loadtxt('LoopH-2-Constraint4.txt')
        y0 = data
        r = interpolate(Constant(float(y0)),RFnSp)
        A_in =  XDMFFile("LoopH-0-Constraint4.xdmf")
        A_in.read_checkpoint(A,"A",0)
        u_in =  XDMFFile("LoopH-1-Constraint4.xdmf")
        u_in.read_checkpoint(u,"u",0)
        assign(Aur,[A,u,r])
   
       (A, u, r) = split(Aur)
       
       F = ( -(1-u**2)*u*du + K1*inner(grad(u), grad(du)) - T1*A.dx(0)*u*du + A**2*u*du + r*du + dr*(2*u-1) + inner(grad(A), grad(dA)) + u**2*A*dA )*dx + H*dA*ds
       solve(F == 0, Aur, bcs,
          solver_parameters={"newton_solver":{"convergence_criterion":"residual","relaxation_parameter":rlx_par,"relative_tolerance":0.001,"absolute_tolerance":tol_abs,"maximum_iterations":50000}})
       ...
       Aur_out = XDMFFile('LoopH-0-Constraint4.xdmf')
       Aur_out.write_checkpoint(Aur_split[0], "A", 0, XDMFFile.Encoding.HDF5, False) #false means not appending to file
       Aur_out.close()
       Aur_out = XDMFFile('LoopH-1-Constraint4.xdmf')
       Aur_out.write_checkpoint(Aur_split[1], "u", 0, XDMFFile.Encoding.HDF5, False) #false means not appending to file
       Aur_out.close()
       with open("LoopH-2-Constraint4.txt", "w") as file:
           print(float(Aur_split[2]), file=file)
   
       if any(abs(mu - plot) <= step for plot in plot_list) :
          fig=plot(u)
          plt.title(r"$u(x)$ for $\mu=$%3.3f, H=%3.3f"%(mu,Hl),fontsize=26)
          #plt.show()
          plt.savefig('H%s/u(x)-H%s-mu%3.3f.png'%(int(Hl*10),int(Hl*10),mu))
          plt.clf()
          fig=plot(A)
          plt.title(r"$A(x)$ for $\mu=$%3.3f, H=%3.3f"%(mu,Hl),fontsize=26)
          plt.savefig('H%s/A(x)-H%s-mu%3.3f.png'%(int(Hl*10),int(Hl*10),mu))
          #plt.show()
          plt.clf()
   ...
   #plt.show()
   plt.savefig('H%s/sigma(d)-H%s.png'%(int(Hl*10),int(Hl*10)))
   plt.clf()
