...
mu_values = np.arange(2*np.pi*step**3, 2*np.pi, step)
surface_energies = []
...
for mu in mu_values:
    ...
    d1 = np.cos(mu)
    d2 = np.sin(mu)
    K1 = float(K11*d1**2 + K22*d2**2 + 2*K12*d1*d2) 
    ...
    F = ( -(1-u**2)*u*du + K1*inner(grad(u), grad(du)) + A**2*u*du + r*du \
    ...
    pie = assemble( (1/Lx)*( (1-u**2)**2/2 + K1*inner(grad(u), grad(u)) \
